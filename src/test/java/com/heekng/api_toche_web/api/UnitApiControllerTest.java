...
@ActiveProfiles("real")
public class UnitApiControllerTest {
    ...
    @Test
    public void unitDetailTest() throws Exception {
        String url = "/api/v1/units/554";

        mockMvc.perform(MockMvcRequestBuilders.get(url).contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.size()").isNotEmpty())
                .andExpect(jsonPath("$.id").value("554"))
                .andDo(print());
    }

    @Test
    public void unitMostItemsTest() throws Exception {
        String url = "/api/v1/units/182/mostItems";
